...
void Cover::handleCoverPosition(ESP8266WebServer *server)
{
...
void Cover::handleCoverSet(ESP8266WebServer *server)
{
...
void Cover::handleCoverSetting(ESP8266WebServer *server)
{
	uint8_t tmp[10];
	int len;
...
void Cover::handleCoverReset(ESP8266WebServer *server)
{
...
{
	boolean buttonState = digitalRead(GPIO_PIN[GPIO_KEY1]);
...
{
	if (getPositionState || perSecond % 60 == 0)
	{
		getPositionTask();
	}
}
...
{
	server->on(F("/cover_position"), std::bind(&Cover::handleCoverPosition, this, server));
	server->on(F("/cover_set"), std::bind(&Cover::handleCoverSet, this, server));
	server->on(F("/cover_setting"), std::bind(&Cover::handleCoverSetting, this, server));
	server->on(F("/cover_reset"), std::bind(&Cover::handleCoverReset, this, server));
}
